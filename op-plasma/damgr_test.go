...
func TestFinalization(t *testing.T) {
	...
	require.NoError(t, state.ExpireCommitments(bID(32)))
	require.Empty(t, state.commitments)
	state.ExpireChallenges(bID(32))
...
func TestExpireChallenges(t *testing.T) {
	...
	for ; i < 3713948; i += 6 {
		...
		comms[i] = comm
		logger.Info("set commitment", "block", i, "comm", hex.EncodeToString(comm.Encode()))
...
func TestDAChallengeDetached(t *testing.T) {
	...
	state.ExpireChallenges(bID(12))
	err := state.ExpireCommitments(bID(12))
	require.NoError(t, err)

	// c1 expires finally
	state.ExpireChallenges(bID(14))
	err = state.ExpireCommitments(bID(14))
...
func TestAdvanceChallengeOrigin(t *testing.T) {
	...
	for i := bn + 1; i < bn+1+pcfg.ChallengeWindow+pcfg.ResolveWindow; i++ {
		id2 := eth.BlockID{
			Number: i,
			Hash:   bhash2,
		}
