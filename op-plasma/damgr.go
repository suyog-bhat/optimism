...
func (d *DA) GetInput(ctx context.Context, l1 L1Fetcher, comm CommitmentData, blockId eth.L1BlockRef) (eth.Data, error) {
	// Check if we have a saved challenge from a look-ahead/previous forwards run.
	status := d.state.GetChallengeStatus(comm, blockId.Number)
	// Skip tracking the commitment if there is an expired challenge so we don't trigger ErrReorgRequired at a later point.
	if status == ChallengeExpired {
		return nil, ErrExpiredChallenge
	}
	// Record the commitment for later finalization / invalidation if it is not immediately expired
...
func (d *DA) loadChallengeEvents(ctx context.Context, l1 L1Fetcher, block eth.BlockID) error {
	...
	for _, log := range logs {
		...
		case ChallengeActive:
			...
			d.log.Info("detected new active challenge", "block", block, "comm", hex.EncodeToString(comm.Encode()))
