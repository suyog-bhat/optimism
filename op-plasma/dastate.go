...
type Commitment struct {
	...
	resolveWindowEnd   uint64         // represents the block number after which the commitment must be resolved by if it was challenged.
}
...
type Challenge struct {
	...
	commResolveWindowEnd     uint64          // block number at which the challenge must be resolved by
...
func (s *State) CreateChallenge(comm CommitmentData, inclusionBlock eth.BlockID, commBlockNumber uint64) {
	c := &Challenge{
		...
		commResolveWindowEnd:     commBlockNumber + s.cfg.ChallengeWindow + s.cfg.ResolveWindow,
...
func (s *State) TrackCommitment(comm CommitmentData, inclusionBlock eth.L1BlockRef) {
	s.commitments = append(s.commitments, Commitment{
		...
		resolveWindowEnd:   inclusionBlock.Number + s.cfg.ChallengeWindow + s.cfg.ResolveWindow,
	})
}
...
func (s *State) ExpireChallenges(origin eth.BlockID) {
	for len(s.challenges) > 0 {
		...
		if c.commResolveWindowEnd > origin.Number {
			return
		}
		s.log.Info("Expiring challenge", "comm", hex.EncodeToString(c.comm.Encode()), "commInclusionBlockNumber", c.commInclusionBlockNumber, "origin", origin)
...
func (s *State) Prune(finalizedBlock eth.BlockID) eth.L1BlockRef {
	...
	for len(s.expiredCommitments) > 0 {
		...
		_, ok := s.GetChallenge(c.comm, c.inclusionBlock.Number)

		// Determine when the challenge expires
		expiresAt := c.challengeWindowEnd
		if ok {
			expiresAt = c.resolveWindowEnd
		}
	...
	for len(s.expiredChallenges) > 0 {
		...
		if c.commResolveWindowEnd > finalizedBlock.Number {
			break
		}
